...
console.log(isRaining === false ? `현재${city}은 ${temperature}이며, 비가 오지 않습니다.` : `현재${city}은 ${temperature}이며, 비가 오고 있습니다.`);
...
num = Number(prompt("0 ~ 100사이의 숫자를 입력하세요"));
magic = (num * 2 + 8) / 2 - num 
...
originalPrice = Number(prompt("원가 입력해주세요"));
discountRate = Number(prompt("할인율을 입력해주세요"));
discountAmount = originalPrice * (discountRate / 100);
finalPrice = originalPrice - discountAmount;

console.log(`최종 가격은 ${finalPrice}입니다.`);
...
// 관계연산자(===) 계산 후에 대입연산자(=)를 진행 boolean 
let hasTrophy = prompt("플레이어가 특별 트로피를 가지고 있나요? (yes/no)")
=== 'yes';

// 사용자로부터 트로피를 가지고 있는지 답변을 받아 변수에 저장 (yes/no의 데이터 입력받기 - yes와 동일할 경우 해당 변수에 true값 저장)

let gameWins = Number(prompt("플레이어가 우승한 게임의 수를 입력하세요"));

let isEligible = score >= 5000 && (hasTrophy == true || gameWins >= 3) ? "yes" : "no";
