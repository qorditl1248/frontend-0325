...
// >> 기본적으로 반복에 대한 기능을 포함
// >> 각 콜백함수의 인자값은 필수 사항 X (value만 사용하는 경우 많음)
//   : 사용하지 않을 경우 작성 X
...
const array = [1, 2, 3];
array[0] = 4;
console.log(array); // 요소값만 변화 O -  참조값 변경 X

// array = [4, 5, 6]; - Error : const 상수에 재할당 불가
// console.log(array); 

// const로 선언된 배열의 요소값이 변경 
// : const가 가지는 불변성은 해당 값이 가지는 !!!!!참조값!!!!! (데이터가 저장되어있는 주소값)
...
// : 어떤 메서드(기능)이 반환(리턴)하는 값을 기반으로 함수를 줄줄이 사용하는 것을 의미
