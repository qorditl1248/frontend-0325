...
//! 1. 함수란? (동일한 동작을 반복, 입력값이 있으면 리턴값을 보여줌)
...
console.log(multiply(2230, 300)); //669000
...
//! 3. 함수 구현 방법 (함수 생성)
// function의 키워드를 사용 - function은 자바스크립트의 예약어 (변수명 사용 불가!)
...
//! 4. 함수 선언 방식 (3가지 방식 다 알아야 함)
//& 함수 선언식(Function Declaration) - 이걸 제일 많이 씀 
// : 함수를 선언하는 가장 전통적인 방식
// : function 키워드로 시작, 함수의 이름이 '필수적으로 명시'
// : 호이스팅이 적용!
//   함수를 선언되기 전에도 호출할 수 있음

// - 호이스팅 (var, let, const 셋 다 호이스팅은 됨 - let이랑 const는 중간에 걸림)
...
function lsa() {
  // 지역 변수(함수 스코프 내에서만 사용)
  let job = 'Developer';
  console.log(job);
  return seungah + job; // 문자열 연산자(+)
}

console.log(lsa()); // seungahDeveloper
