...
// 할 일 목록을 저장할 배열을 초기화
// : 할 일 목록의 요소값은 객체 (todos 라는 빈 배열을 초기화) 
let todos = [];
...
function addTodo(content) {
  const todo = {
    id: todos.length + 1, // 새 할 일에 고유 id 할당 (배열의 길이 + 1로 설정) 0이 아닌 1의 값으로 시작할려고 
    content: content,
    completed: false, // 할 일의 완료 상태를 기본적으로 '미완료'로 지정
  };
  todos.push(todo); // 생성한 할 일 즉, todo 객체를 todos 배열에 추가

  //? 새로운 할 일이 추가될 때 마다 할 일 목록을 콘솔에 출력
  displayTodos();
}

//& 할 일을 삭제하는 함수 deleteTodo
// : 주어진 ID를 가진 할 일을 todos 배열에서 제거
// >> 명시적으로 구분되는게 'id' 값

function deleteTodo(id) { // id : 제거할 요소의 id
  // 주어진 id를 가진 할 일을 제외하고, 새로운 배열로 생성 
  todos = todos.filter(todo => todo.id !== id);  // todos는 배열 / todo는 객체 

  //? 변경된 할 일 목록을 콘솔에 출력 
  displayTodos();
}

//& 할 일의 완료 상태를 변경하는 함수 toggleTodoComplete
...
console.log(lsa); // { name: '이도경', height: 169 }
console.log(lsa); // { name: '이도경', height: 169, completed: false }
