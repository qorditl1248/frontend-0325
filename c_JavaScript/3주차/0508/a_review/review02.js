...
function addToCart(id, name, price, quantity) {
  // 배열.findindex();
  // : 해당 배열에 일치하는 인덱스가 있는 경우 - 해당 인덱스 반환 
  // : 해당 배열에 일치하는 인덱스가 있는 경우 - -1 
...
function updateQuantity(id, quantity) {
  const index = cart.findIndex(item => item.id === id); // 상품의 id가 일치하는 상품의 인덱스를 활용
...
addToCart(1, '사과', 0.99, 2); // quantity 즉, 수량만 업데이트 됨 
