//! 동기 처리 프로그래밍(synchronous)
{
  ...
  function performAnotherCalculation(): number {
    ...
    return product; // 계산 결과를 반환하는 식 
  }

  // 동기적 처리를 수행하는 함수  
  function syncFunc() {
    console.log('첫 번째 작업 시작');
    let result = performCalculation(); // 시간이 오래걸리는 작업 
    console.log(`첫 번째 작업 완료: ${performCalculation()}`);

    console.log('두 번째 작업 시작');
    let anotherResult = performAnotherCalculation();
    console.log(`두 번째 작업 완료: ${performAnotherCalculation()}`);
  }
