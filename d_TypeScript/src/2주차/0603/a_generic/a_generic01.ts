// a_generic01.ts
{
  ...
  // 컴파일(Compile)
  // : 소스코드를 작성하고 편집하는 과정, 코드를 입력하고 있는 과정 

  // 런타임(Runtime)
  // : 컴파일 과정을 마친 프로그램이 사용자에 의해 실행되어 동작되어지는 때, 실행 누르면
  ...
  function generic<T>(arg: T): T {
    ...
    // : 해당 타입 변수는 입력 인수의 타입을 T로 지정, 함수의 반환 타입을 T로 지정
    return arg;
  }

  let stringOutput = generic<string>("안녕하세요");
  // let stringOutput = generic("안녕하세요"); - 사용 가능 
  // let stringOutput2 = generic<string>(500); - Error, 우선순위는 꺽쇠괄호 안에 있는거 

  let numberOutput = generic<number>(500); 
  // let numberOutput = generic(500); - 사용 가능 
  ...
  //# 1) 제네릭 함수
  function genericFunc<T>(args: T[]): T[] { // string[], number[] - 배열 타입 명시
  ...
  let myIdentity: IGeneric<number> = indentity;
  console.log(indentity(5)); // 5
  console.log(indentity("안녕")); // 안녕
