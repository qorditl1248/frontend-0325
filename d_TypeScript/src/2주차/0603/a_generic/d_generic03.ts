// d_generic03.ts
{
  ...
  // user.name = 'lsa'; - Error 읽기 전용이라 변경 불가능 
  
  //? 유니온 타입과 제네릭 함수 작성 예시
  // 문자열 배열과 숫자 배열 중 하나를 입력받아
  // , 배열의 첫 번째 요소를 반환하는 제네릭 함수 구현

  function getFirstElemnet<T extends string | number>(array: T[]):string | number {
  ...
  const firstString = getFirstElemnet(["apple", "banana", "orange"]);
  const firstNumber = getFirstElemnet([1, 2, 3]);
