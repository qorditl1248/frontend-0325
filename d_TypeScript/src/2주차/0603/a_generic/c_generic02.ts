// c_generic02.ts
{
  ...
  let variable = constraints({
    length: 10,
    // length 속성을 가진 객체이지만, 추가로 기타 속성 명시가 가능! 
    // 그 이유는, 확장이라고 명시를 함 extends 
    value: 3,
    addedOption: '안녕하세요'
  })
  ...
  function findMin<T extends Index, K extends keyof T>(array: T[], key: K) {
    // array 배열 내에서 푀소 값을 가진 객체를 찾기 

    return array.reduce((min, item) => { // min은 축적값, item은 돌면서 값을 만들어옴 
  ...
  function checkType<T>(value: T): Check<T> {
    // typeof로 하면 boolean 값으로 반환되기 때문에 밑에 삼항연산자로 
