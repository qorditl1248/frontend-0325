...
// 1) 우선 순위 : 여러 연산자 사용 시, 어느 연산자를 먼저 처리하게 될 것인지 결정
let i = 3 + 4 * 5;

// 2) 결합 방식 : 수식 내에 우선순위가 같은 연산자가 둘 이상 있을 때, 먼저 어느 연산을 수행할 것인가를 결정 
// >> 각각의 연산자 종류에 따라 사이(다름)
...
x = 10; 
console.log(x++); // 후위 증감 연산자 : +1이 해당 값에서 바로 적용되지 않음 
console.log(x);
...
//* '=='와 '==='의 차이
// : 동등 연산자는 두 값이 같은지 비교할 때 자동으로 타입 변환을 수행(내용만 비교), 외관만 비교
