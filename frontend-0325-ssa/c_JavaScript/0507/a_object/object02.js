...
// >> Node.js 에서 global(전역)객체
// >> 브라우저 환경에서는 window

//! >> 2. 함수 컨텍스트 (중요)

//? 일반 함수의 this
// : 전역 객체를 의미 - 전역 컨텍스트와 동일하게 사용
...
myObject.showThis(); 
// { name: 'object', showThis: [Function: showThis] } - myObject 그 자체를 가리킴
...
const arrowMyObject = {
  name: 'object',
  // 해당 화살표 함수가 정의되는 생성 스코프를 가지는 컨텍스트를 가지고 옴
  showThis: () => {
    console.log(this);
  }
}
