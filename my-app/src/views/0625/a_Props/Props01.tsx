...
function Data({name}: DataType) {  
  return (
    <div>안녕하세요, {name}님 </div>
  )
}

// 위 값이랑 똑같음 위는 구조 분해 한 것 
...
// : 컴포넌트명.defaultProps = { 기본속성: '기본속성값' }
  ...
  Hello.defaultProps = { 
    name: '등록되지 않는 고객' // 아무 값도 넣지 않으면 이 값이 나옴 
  }
...
const Wrapper: React.FC<childrenType> = ({children}) => {
  ...
  return (
    <div style={style}>
      ...
      >>> 단일 태그 컴포넌트로 사용 x
          : 열리고 닫히는 태그 사이에 내용으로 node요소(컴포넌트)가 포함되어있어야 함 
      */}
      {children}
    </div>
  )
...
export default function Props01() {
  return (
    // 자식 컴포넌트를 호출 >> 자식 컴포넌트에게 필요한 props를 전달!
    <Wrapper>
      <Data name='이도경'/>
